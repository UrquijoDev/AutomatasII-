...
public class Sintactico {
    ...
    public void sintaxis(String nombreArchivo) {
                            ...
                            if (p.idToken == 123) // {
                            {
                                ...
                                while (p != null && errorSintactico == false) { // nodo de procesamiento no nulo
                                    ...
                                    } else if (p.idToken == 124) // }
                                    {
                                        ...
                                        resultadoBuilder.append("Inicio de sentencia invalido en ").append(p.linea)
                                                .append("\n");
        ...
        if (errorSintactico == false) {
            if (contadorCorchetes == 0) {
                ...
                // Generar ASM si no hay errores y está activado
                if (generarCodigoIntermedio) {
                    GeneradorASM generadorASM = new GeneradorASM();
                    generadorASM.generarArchivoASM(nombreArchivo, generadorCodigo.getCodigoOptimizado(), cabezaVar);
                }
            ...
            while (p != null && p.sig != null) {
                ...
                } else if (p.idToken == 100) // identificador (Iniio de asignacion de variable)
                {
                    ...
                    if (!existeVariable(p.lexema)) {
                        resultadoBuilder.append("Error semántico: Variable '").append(p.lexema)
                                .append("' no declarada (línea ").append(p.linea).append(")\n");
                        errorSintactico = true;
                    }
                    ...
                    if (p.idToken == 113) // =
                    {
                        ...
                        if (checkExpreSimple()) {
                            ...
                            if (!errorSintactico
                                    && !notacionPolish.validarAsignacion(tipoVar, (p != null ? p.linea : renglon))) {
                    ...
                    if (p.idToken == 117) // (
                    {
                        ...
                        if (p.idToken == 100 || p.idToken == 126 || p.idToken == 127) // Id,Cadena o Char
                        {
                            ...
                            if (p != null && p.idToken == 118) // ) )
                            {
                                ...
                                resultadoBuilder.append("Se espera ) en ").append(p != null ? p.linea : "desconocida")
                                        .append("\n");
                                errorSintactico = true;
                            }
                        } else {
                            resultadoBuilder.append("Se espera un identificador o cadena valida en linea ")
                                    .append(_renglon).append("\n");
                            errorSintactico = true;
                        }
    ...
    private void insertarVariable(String nombre, int tipo) {
        if (existeVariable(nombre)) {
            resultadoBuilder.append("Error semántico: Variable '").append(nombre).append("' ya declarada (línea ")
                    .append(p.linea).append(")\n");
            errorSintactico = true;
            return;
        }
    ...
    private boolean checkOperacionRelac() {
        if (p.idToken == 108 || p.idToken == 109 || p.idToken == 110 || p.idToken == 111 || p.idToken == 112
                || p.idToken == 116 || p.idToken == 114 || p.idToken == 115) {
            return true;
        } else {
            return false;
        }
    ...
    private boolean checkFactor() {
        ...
        if (p.idToken == 100) { // id
            if (!variableDeclarada(p.lexema)) {
                resultadoBuilder.append("Error semántico: Variable '").append(p.lexema).append("' no declarada (línea ")
                        .append(p.linea).append(")\n");
                errorSintactico = true;
            } else {
            ...
            while (p != null && (checkOperacionRelac() || checkOperacionLogica())) {
                if (checkOperacionRelac()) {
                    ...
                    if (checkExpreSimple()) {
                        ...
                        errorSintactico = true;
                        resultadoBuilder.append("Se espera expresión simple después del operador relacional en línea ")
                                .append(p.linea).append("\n");
                        break;
                    }
                    ...
                    if (checkExpreSimple()) {
                        ...
                        errorSintactico = true;
                        resultadoBuilder.append("Se espera expresión simple después del operador lógico en línea ")
                                .append(p.linea).append("\n");
                        break;
                    }
                }
